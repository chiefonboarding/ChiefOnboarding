<template>
  <v-col sm="12" mb="6" offset-mb="3" md="8" offset-md="2">
    <v-row class="mb-4">
      ...
      <v-col sm="6" class="text-right" style="margin-top: 12px;">
        <CompletedTasksDialog by-user />
...
export default {
  ...
  mounted () {
    ...
    this.$hrtasks.getAll().then((tasks) => {
      this.allTasks = tasks
      this.tasks = tasks.filter(a => a.assigned_to !== null && a.assigned_to.id === this.$store.state.admin.id && !a.completed)
    }).finally((tasks) => {
      this.loading = false
    })
  },
