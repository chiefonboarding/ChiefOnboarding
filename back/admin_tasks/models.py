...
class AdminTask(models.Model):
    new_hire = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='new_hire_tasks')
    assigned_to = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='owner',
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new and self.option == 1:
            ...
            blocks = [{
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": self.assigned_to.full_name() + " needs your help with this task:\n*" + self.title + "*\n_" + self.comment.last().content + "_"
                }
            }, {
                {
                    "type": "actions",
        ...
        if self.admin_task.assigned_to != self.comment_by:
            if self.admin_task.assigned_to.slack_user_id != "" and team is not None:
                blocks = [{
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": self.comment_by.full_name() + " added a message to your task:\n*" + self.admin_task.title + "*\n_" + self.comment + "_"
                    }
                }, {
                    {
                        "type": "actions",
