...
def extract_questions_from_pdf(request):
    ...
    try:
        ...
        # Try multiple approaches to extract questions

        # Approach 1: Extract text with PyPDF2 and send to OpenAI
        try:
            logger.info("Trying text extraction with PyPDF2")
            ...
            if pdf_text.strip():
                ...
                if questions and len(questions) > 0:
                    logger.info(f"Successfully extracted {len(questions)} questions using PyPDF2 text extraction")
                    return JsonResponse({'questions': questions})
                else:
                    logger.info("No questions extracted using PyPDF2 text extraction, trying alternative approach")
            else:
                logger.info("No text extracted using PyPDF2, trying alternative approach")
        except Exception as e:
            logger.error(f"Error with PyPDF2 extraction: {str(e)}")
            logger.info("Trying alternative approach")

        # Approach 2: Send the PDF directly to OpenAI Vision API
        try:
            logger.info("Trying direct PDF analysis with OpenAI Vision API")
            ...
            if vision_questions and len(vision_questions) > 0:
                ...
                logger.warning("No questions extracted using Vision API")
